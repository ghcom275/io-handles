...
use io_handles::{BufReaderLineWriter, BufReaderWriter};
use std::{
    io::{self, prelude::*, ErrorKind, IoSlice},
    panic,
...
fn test_buffered_reader() {
    ...
    let mut reader = BufReaderWriter::with_capacities(2, 2, io::Cursor::new(inner));
...
fn test_buffered_reader_invalidated_after_read() {
    let inner: Vec<u8> = vec![5, 6, 7, 0, 1, 2, 3, 4];
    let mut reader = BufReaderWriter::with_capacities(3, 3, io::Cursor::new(inner));
...
fn test_buffered_writer() {
    let inner = Vec::new();
    let mut writer = BufReaderWriter::with_capacities(2, 2, io::Cursor::new(inner));
...
fn test_buffered_writer_inner_flushes() {
    let mut w = BufReaderWriter::with_capacities(3, 3, io::Cursor::new(Vec::new()));
    w.write(&[0, 1]).unwrap();
...
#[test]
fn test_line_buffer() {
    let mut writer = BufReaderLineWriter::new(io::Cursor::new(Vec::new()));
...
fn test_read_line() {
    ...
    let mut reader = BufReaderWriter::with_capacities(2, 2, io::Cursor::new(in_buf));
...
fn test_lines() {
    let in_buf: Vec<u8> = b"a\nb\nc".to_vec();
    let reader = BufReaderWriter::with_capacities(2, 2, io::Cursor::new(in_buf));
...
fn test_short_reads() {
    ...
    let mut reader = BufReaderWriter::new(inner);
    let mut buf = [0, 0];
    assert_eq!(reader.read(&mut buf).unwrap(), 0);
...
fn dont_panic_in_drop_on_panicked_flush() {
    ...
    let _writer = BufReaderWriter::new(writer);
...
fn panic_in_write_doesnt_flush_in_drop() {
    ...
    thread::spawn(|| {
        let mut writer = BufReaderWriter::new(PanicWriter);
...
fn bench_buffered_reader(b: &mut test::bench::Bencher) {
    b.iter(|| BufReaderWriter::new(Empty));
}
...
fn bench_buffered_writer(b: &mut test::bench::Bencher) {
    b.iter(|| BufReaderWriter::new(Sink));
}

/// A simple `Write` target, designed to be wrapped by `BufReaderLineWriter` /
/// `BufReaderWriter` / etc, that can have its `write` & `flush` behavior
/// configured
...
fn erroneous_flush_retried() {
    ...
    // to the last newline.
    let mut writer = BufReaderLineWriter::new(writer);
...
#[test]
fn line_vectored() {
    let mut a = BufReaderLineWriter::new(io::Cursor::new(Vec::new()));
    assert_eq!(
        a.write_vectored(&[
            IoSlice::new(&[]),
            IoSlice::new(b"\n"),
...
fn line_vectored_partial_and_errors() {
    ...
    let mut a = BufReaderLineWriter::new(Writer::default());
...
fn long_line_flushed() {
    let writer = ProgrammableSink::default();
    let mut writer = BufReaderLineWriter::with_capacities(5, 5, writer);
...
fn partial_multiline_buffering() {
    ...
    let mut writer = BufReaderLineWriter::with_capacities(10, 10, writer);
